...
description: Suppress specific vulnerability findings using .olympix-ignore.json files and ignore comments.
---
...
:::note
Each entry can be thought of as the following regex: `^ENTRY.*`, where the string `ENTRY` is replaced with each entry in the provided array. Any file matching any of these regexes would be ignored. Hence, these paths can be partial paths too, the asterisk at the end is implied, no wildcards are allowed.
:::
